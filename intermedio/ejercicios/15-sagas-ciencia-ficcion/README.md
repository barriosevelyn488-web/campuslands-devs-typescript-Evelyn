...
Vas a resolver un caso relacionado con ciencia ficcion: **Analizador de sagas sci-fi**. La idea es practicar TypeScript como se usa en proyectos reales: primero entiendes los datos, luego defines tipos claros y finalmente implementas la logica.
...
1. Modela el dominio de ciencia ficcion con tipos e interfaces claras.
2. Crea datos de prueba realistas para Analizador de sagas sci-fi.
3. Implementa funciones separadas para validar, transformar y calcular resultados.
4. Usa arrays tipados y map sin recurrir a any.
...
```text
intermedio/ejercicios/15-sagas-ciencia-ficcion/resoluciones/nombre-apellido/
```

Ejemplo valido:

```text
intermedio/ejercicios/15-sagas-ciencia-ficcion/resoluciones/juan-perez/index.ts
```
...
Conceptos recomendados: `arrays tipados`, `map`, `filter`, `sort`.
