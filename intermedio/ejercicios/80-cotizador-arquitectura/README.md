...
diseno 3D arquitectura

## Contexto del problema

Vas a resolver un caso relacionado con diseno 3D arquitectura: **Cotizador arquitectura 3D**. La idea es practicar TypeScript como se usa en proyectos reales: primero entiendes los datos, luego defines tipos claros y finalmente implementas la logica.
...
1. Modela el dominio de diseno 3D arquitectura con tipos e interfaces claras.
2. Crea datos de prueba realistas para Cotizador arquitectura 3D.
3. Implementa funciones separadas para validar, transformar y calcular resultados.
4. Usa arquitectura simple y DTO sin recurrir a any.
...
```text
intermedio/ejercicios/80-cotizador-arquitectura/resoluciones/nombre-apellido/
```

Ejemplo valido:

```text
intermedio/ejercicios/80-cotizador-arquitectura/resoluciones/juan-perez/index.ts
```
...
Conceptos recomendados: `arquitectura simple`, `DTO`, `repositorio en memoria`, `reportes`.
